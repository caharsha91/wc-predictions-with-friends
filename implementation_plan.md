...
- [ ] **Admin Safety (Partially Complete):**
    - [x] Create a reusable `ConfirmationModal` component.
    - [ ] Wrap destructive actions in `/admin` (Seed Database, Reset) with this modal to prevent accidental data loss.
    - [ ] Add a distinct "Reset to Live" button in the Demo Controls tab. *(Blocked: requires admin page components)*
...
+
+**Theme Support:**
+- Check contrast ratios for the "Sticky Row" highlight color in Dark Mode.
+- Ensure the "Rivalry Strip" text (especially negative/positive deltas) is color-coded accessibly.
+
...
+
+**Theme Support:**
+- Ensure Stepper buttons have sufficient touch targets (min 44px) and clear active states in all themes.
+- Verify that "List View" cards maintain clear separation in Dark Mode.
+
...
+
+**Theme Support:**
+- Design badges using SVG icons that adapt fill colors based on the active theme (Light/Dark).
+- Ensure the "What-If" simulator UI clearly distinguishes between "Live" data and "Simulated" data (e.g., using a dashed border or distinct background tint).
